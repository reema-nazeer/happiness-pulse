#!/bin/bash
# Homey Happiness Pulse — install for the Technology team.
#
# Self-contained: usable as a single download / one-line curl. Run with:
#
#   curl -sL https://raw.githubusercontent.com/reema-nazeer/happiness-pulse/main/Scripts/install-technology.sh | bash
#
# What this does:
...
curl -sL -X POST \
  ...
  -d "{\"type\":\"install\",\"username\":\"$(whoami)\",\"source\":\"install-technology-v3\",\"timestamp\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"arch\":\"$(uname -m)\",\"os\":\"$(sw_vers -productVersion)\",\"department\":\"$PULSE_DEPARTMENT\"}" \
  "$PULSE_WEBHOOK_URL" \
  > /dev/null 2>&1 || true
